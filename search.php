...
$req1 = "select * from job order by city";
...
if (mysqli_num_rows($env)==0){
    ...
    while($res=mysqli_fetch_row($env)){ ?>
     <?php  $req2 = "SELECT `name`, `img` FROM `inscription` WHERE `email`='$res[3]'";
        ...
        ?>
        <form method="POST" action="select-job.php" >
        <table class='record' border='0' > 
        
          <tr> <th class="mt"><b><img class='profile-pic' src='profile/<?php echo $res2[1] ?>'></b></td> <td class='td2'><b>name: </b><?php echo $res2[0] ?></td> </tr>
        ...
        <tr> <th class="rt"><b> salery :</b></td> <td class='td2'> <?php echo $res[6]  ?>DT </td> </tr>
        <tr id="box"> <th class="rt"><b> discription:</b></td> <td class='td2'>  <?php echo $res[7] ?> </td> </tr>
      <tr class="inv"> <td ><input type="hidden"  name="sel" value=" <?php echo $res[8] ?>">  </td> <td >
      <button type='submit' value='".$res[9]."'>select
      </button> </td> </tr>
      </table>
    </form>
      <?php }
